(function(){
    ...
    _p.load = function(_useServerData, list, cb)
    {
        ...
        function loadImage()
        {
            ...
            img.onload = function()
            {
                WireGraphic.addData(id, data, img, img.width, img.height);
                index ++;
                loadOne();
            };
...
(function (){
    ...
    (function ()
    {
        ...
        _p.addData = function (id, data, image, rawWidth, rawHeight)
        {
        ...
        _p.updateDataGeom = function(id, left, top, width, height)
        {
            var data = __graphicDic[id];
            ...
            data.scaleX = data.width / data.rawWidth;
            data.scaleY = data.height / data.rawHeight;
        };
        ...
        _p.tweenTo = function(startId, targetId, setting, cbGraphicArrived, cb)
        {
            ...
            __currentGraphicId = startId;
            __nextGraphicId = targetId;

            var g1 = __graphicDic[startId];
            var g2 = __graphicDic[targetId];


            if(setting.startBound) _stageBound = setting.startBound;
            else
            {
                _stageBound = {left:g1.left, top:g1.top, scaleX:g1.scaleX, scaleY:g1.scaleY};
            }

            if(setting.targetBound) _targetBound = setting.targetBound;
            else
            {
                _targetBound = {left:g2.left, top:g2.top, scaleX:g2.scaleX, scaleY:g2.scaleY};
            }
            ...
            //if(!setting || !setting.d1) setting.d1

            var d1 = !setting.d1? 1.5: setting.d1;
            var d2 = !setting.d2? 2: setting.d2;
            var d3 = !setting.d3? 1.3: setting.d3;
            var d4 = !setting.d4? .7: setting.d4;

            //console.log("d1 = " + setting.d1);
            //console.log("d1 = " + setting.d2);
            //console.log("d1 = " + setting.d3);
            //console.log("d1 = " + setting.d4);
            ...
            tl.to(__tweenObj, d1, {progress:.5, ease:setting.outFunc, onUpdate:onProgressUpdate});
            tl.to(__tweenObj, d2, {progress:.6, ease:Linear.easeNone, onUpdate:onProgressUpdate});
            tl.to(__tweenObj, d3, {progress:1, ease:setting.inFunc, onUpdate:onProgressUpdate});
            ...
            tl.add(function()
            {
               if(cbGraphicArrived) cbGraphicArrived.apply();
            });
            tl.to(PointLayer.object3D.material.uniforms.opacity, d4, {value:0});
            tl.to(LineLayer.object3D.material.uniforms.opacity, d4, {value:0}, "-=" + d4);
        ...
        _p.updatePosition = function()
        {
            ...
            {
                _container.position.x = g1.left;
                _container.position.y = g1.top;

                _container.scale.x = g1.scaleX;
                _container.scale.y = g1.scaleY;
